...
;(function(G, lib){
	...
	var doc = G.document,
		...
		dpr = !!dfDpr ? dfDpr : G.devicePixelRatio ? G.devicePixelRatio === Math.floor(G.devicePixelRatio) ? G.devicePixelRatio : 1 : 1,
		scale = 1 / dpr,
	...
	// 设置根字体大小
	function trans(){
		// (G.devicePixelRatio ? G.devicePixelRatio > Math.floor(G.devicePixelRatio) ? 1 : G.devicePixelRatio : 1) !== dpr && dprChange();
		(G.devicePixelRatio ? G.devicePixelRatio === Math.floor(G.devicePixelRatio) ? G.devicePixelRatio : 1 : 1) !== dpr && G.location.reload();
		var w = getW();
		w > maxW && (w = maxW);
